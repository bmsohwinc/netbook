...
class HttpClient(QuicConnectionProtocol):
    ...
    async def migrate(self, new_port: int) -> None:
        """
        Migrate the connection to a new local port.
        """
        if not self._peer_addr:
            logger.error("Peer address is not set, cannot migrate.")
            return

        loop = asyncio.get_running_loop()
        logger.info(f"Attempting to migrate to new port: {new_port}. Peer is {self._peer_addr}")

        # Create a new "connected" UDP socket.
        # Using remote_addr makes sure this new socket knows where to send data.
        new_transport, _ = await loop.create_datagram_endpoint(
            lambda: self,
            local_addr=("127.0.0.1", new_port),
            remote_addr=self._peer_addr
        )

        # Close the old transport and switch to the new one
        self._transport.close()
        self._transport = new_transport
        logger.info("Client transport switched to new socket.")

        # Send a PING on the new path to trigger the server-side migration
        logger.info("Pinging on new path to trigger migration.")
        self._quic.send_ping(0)
        self.transmit()
        logger.info("Transmit called on new path.")
    ...
    def datagram_received(self, data: bytes, addr: tuple) -> None:
        ...
        """
        # Capture the peer address from the first incoming datagram.
        if self._peer_addr is None:
            host, port = addr[0], addr[1]
            ipv6_prefix = "::ffff:"
            if host.startswith(ipv6_prefix):
                host = host[len(ipv6_prefix):]

            self._peer_addr = (host, port)
            logger.info(f"Peer address captured: {self._peer_addr}")

        # Pass the datagram to the parent class to handle QUIC logic.
        super().datagram_received(data, addr)
    
    # Add this method override inside your HttpClient class.

    def transmit(self) -> None:
        """
        Override of the parent transmit method.

        This version is a copy of the base implementation with a modification
        to handle both connected (post-migration) and unconnected (pre-migration)
        sockets. It also includes the critical timer logic.
        """
        self._transmit_task = None

        # --- Start of modified section ---
        if self._transport is not None:
            # Check if the socket is "connected" by seeing if a peername is set.
            ...
            for data, addr in self._quic.datagrams_to_send(now=self._loop.time()):
                try:
                    # If the socket is connected, the addr argument to sendto MUST be None.
                    # Otherwise, we use the address provided by the QUIC stack.
                    destination_addr = None if is_connected else addr
                    self._transport.sendto(data, destination_addr)
                except OSError as exc:
                    # This can happen if the socket is closed unexpectedly.
                    logger.warning(f"Could not send datagram: {exc}")
        # --- End of modified section ---

        # --- Start of copied timer logic (from base class) ---
        # This part is essential for the QUIC connection to function.
...
async def perform_http_request(
    ...
    for http_event in http_events:
        if isinstance(http_event, DataReceived):
            http_event_data = http_event.data
            print(f"Received data: {http_event_data}")
            octets += len(http_event_data)
    logger.info(
...
async def trigger_migration(client: HttpClient, new_port: int) -> None:
    await asyncio.sleep(5)
    logger.info("------- MIGRATION TRIGGERED -------")
    await client.migrate(new_port)
...
async def main(
    ...
    async with connect(
        ...
        # Trigger client migration
        if local_port != 0:
            new_local_port = local_port + 1
            asyncio.create_task(trigger_migration(client, new_local_port))
